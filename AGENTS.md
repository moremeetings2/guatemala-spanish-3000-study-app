...
- Study persistence includes the active card ID and owning account ID. Closing and reopening an iPhone/PWA study session must resume the same card even though the surrounding order is reshuffled at boot; account-scoped My Words cards resume after `loadMyWords` completes. Separate per-account snapshots retain local-only reading/resume history across logout, and delayed API responses must never mutate a signed-out or different account.
...
- `data/essential_200.json`: canonical 200-word starter deck and companion spreadsheet content (word, meaning, category, Spanish/English sentence). The app reuses 190 Main 3000 IDs with shared progress, overrides their meanings/examples with these reviewed entries, and adds ten missing basics as stable `essential-*` IDs. Total built-in unique cards: 3,609. Access via You → Essential 200 → Study these / Quiz these. Essential quiz distractors remain within this deck. Preserve IDs and exactly 200 unique words when editing.
- `data/sentences.json`: example sentences for all 3000 Main 3000 words (the "Use" button); top ~150 hand-authored, rest generated
- The "You" tab presents consolidated study decks: **Everyday Conversation** (230 cards from the coffee, conversation-verbs, and everyday-phrases source collections), **Guatemalan Lexicon** (369 cards from 356 lexicon entries plus 13 Guatemala usage notes), and **Most Common in Guate** (a live view of the signed-in learner's starred Guatemalan Lexicon cards). Source collection IDs remain unchanged in the study-pack JSON; `app.js` maps them to the product-facing decks while preserving card IDs and synced progress.
- The "You" tab includes a **Country Lexicons** reference view (`renderLexicon`, route `lexicon`): flag chips select any of the 21 Spanish-speaking countries. Guatemala (default) shows the 369-entry studyable in-catalog lexicon deck; the other 20 countries come from `data/country_lexicons.json` (2,275 authored entries — tiered: 150 for MX/CO/VE/CL/AR/ES, 149 PE, 100 for the 12 standard-tier countries, 26 for GQ — with categories + example sentences, loaded at boot into `data.COUNTRY_LEX`). Search, audio, and AI-chat (country-aware prompts) work per country. Content policy: high-confidence documented regionalisms only; agents deliver fewer rather than stretch; native-speaker review required before a country's module launch.
...
- `tests/auth.spec.js`: Playwright tests for the accounts/landing flow (landing, signup, login, logout, account isolation, no-guest gating) with a mocked backend
